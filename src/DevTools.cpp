...
constexpr static auto resource_dir = const_join_path<geode_directory, geode_resource_directory>;
    ...
    SWITCH_ARGS {
        HANDLER("goto") {
            if (!args.hasArg(1)) {
                ...
                if (scene) {
                    Interface::mod()->log() << "Moving to scene " << args.at(1) << geode::endl;
                    CCDirector::sharedDirector()->replaceScene(CCTransitionFade::create(.5f, scene));
                    this->m_commandSuccess = true;
                } else {
                    Interface::mod()->log() << Severity::Error <<
                        "Invalid Command: \"" << args.at(1) << "\" is not a valid SceneID" << geode::endl;
                }
            }
        }
