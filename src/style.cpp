...
void DevTools::loadTheme(DevToolsTheme theme) {
    ...
    switch (theme) {
        case DevToolsTheme::Light: {
            ...
            *this->m_colorYes      = { .0f, 1.f,  .2f, .8f };
            *this->m_colorNo       = { 1.f, .0f,  .0f, .8f };
            *this->m_colorWarning  = { 1.f, .56f, .0f, .8f };
...
void DevTools::loadStyle() {
    if (this->m_loadedStyle) {
        return;
    }
    
    this->m_loadedStyle = true;
